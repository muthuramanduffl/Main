...
public partial class adminkey2hcom_AddPaymentSchedule : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Request.QueryString["PSID"] != null)
            {
                ...
                lblDisplayText.Text = "Edit Payment Schedule";
            }
            ...
            else
            {
                ...
                lblDisplayText.Text = "Add Payment Schedule";
            }
        }
    }
    public void Bind(string ID)
    {
        ...
        {
            CE.StoreExceptionMessage("add-payment-schedule.aspx", "Bind", ex.Message, "Not Fixed");
        }
    }
    public void BindFlatID(string ID)
    {
        ...
        {
            CE.StoreExceptionMessage("add-payment-schedule.aspx", "BindFlatID", ex.Message, "Not Fixed");
        }
    }
    ...
    {
        int txtpercentage = 0;
        if (!string.IsNullOrEmpty(ddlPaymentStages.SelectedValue) && !string.IsNullOrEmpty(txtPercentage.Text) && !string.Equals(txtPercentage.Text, "0"))
        {
            if (int.TryParse(txtPercentage.Text, out txtpercentage) && txtpercentage <= 100)
            {                     
                try
                {
                    int ret = KCD.CheckFlatCostDetailsExistAllCostLabelParameterByFlatID(Convert.ToInt32(ddlFlatNumber.SelectedValue));
            ...
            {
                lblenterpercentageMessage.Text = "Enter a valid percentage (0-100)";
            }
        }
        else
        {
            ...
            lblenterpercentageMessage.Text = string.Equals(txtPercentage.Text, "0") ? "Enter valid percentage" : "";
            ClearTextFields();
        }
    }
    ...
    public static string DeleteShedulestages(string hdnPSID, string selectedValue)
    {
        ClientDashboardError CE = new ClientDashboardError();
        ...
        try
        {
            if (!KNFN.ViewAvailtransactionReceipt(hdnPSID, Userid))
            {
                ret = Ky.DeleteShedulepaymentstages(Convert.ToInt32(hdnPSID));
                if (ret > 0)
                {
                    strisdelete = "Payment schedule and demand details has been deleted";
                }
                else
                {
                    strisdelete = "Payment schedule details has been not deleted";
                }
            }
            else
            {
                strisdelete = "Sorry, you can't delete as receipts have been sent to the customer.";
            }
        }
        catch (Exception ex)
        {
    ...
    {
        Response.Redirect("add-demand.aspx?FlatID=" + ddlFlatNumber.SelectedValue);
    }
    public void VisibleGobtn()
    {
        if (!string.IsNullOrEmpty(ddlFlatNumber.SelectedValue))
        {
            ...
            {
                divbtngonext.Style.Add("display", "Block");
            }
            else
            {
                divbtngonext.Style.Add("display", "none");
