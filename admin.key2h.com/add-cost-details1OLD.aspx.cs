...
public partial class adminkey2hcom_AddCostDetails1 : System.Web.UI.Page
{
    ...
    public void BindFlatID(string ID)
    {
        ...
        try
        {
            ...
            {
                dt = KD.ViewAllFlatCostDetails("", ID, "", "");
            }        
            if (dt != null && dt.Rows.Count > 0)
            {
                if (!string.IsNullOrEmpty(dt.Rows[0]["ProjectID"].ToString()) && dt.Rows[0]["ProjectID"] != null)
    ...
    {
        Response.Redirect("add-payment-schedule1.aspx?FlatID="+ddlFlatNumber.SelectedValue);
    }
}