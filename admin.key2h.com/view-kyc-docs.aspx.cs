...
public partial class adminkey2hcom_ViewKYCDocs : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            CE.StoreExceptionMessage("view-kyc-docs.aspx", "Page_Load", ex.Message, "Not Fixed");
        }
    }
    public void ddlBindProject()
    {
        ...
        {
            CE.StoreExceptionMessage("view-kyc-docs.aspx", "ddlBindProject", ex.Message, "Not Fixed");
        }
    }
    ...
    public string Bindproject(int ID)
    {
        ...
        {
            CE.StoreExceptionMessage("view-kyc-docs.aspx", "Bindproject", ex.Message, "Not Fixed");
        }
        return Project;
    }
    public string BindBlockname(int ID)
    {
        ...
        {
            CE.StoreExceptionMessage("view-kyc-docs.aspx", "BindBlockname", ex.Message, "Not Fixed");
        }
        return Block;
    }
    public string ViewFlatNameByFlatID(int ID)
    {
        ...
        {
            CE.StoreExceptionMessage("view-kyc-docs.aspx", "ViewFlatNameByFlatID", ex.Message, "Not Fixed");
        }
        return Block;
    }
    public void Bind()
    {
        ...
        {
            CE.StoreExceptionMessage("view-kyc-docs.aspx", "Bind", ex.Message, "Not Fixed");
        }
    }
    public DataTable Get()
    {
        ...
        try
        {
            ...
            dt = KYC.ViewAllFlatCustomerKYCDocs(projectid, blockID, FlatID, "", clientId);

        }
        catch (Exception ex)
        {
            CE.StoreExceptionMessage("view-kyc-docs.aspx", "Get", ex.Message, "Not Fixed");
        }
        return dt;
    }
    protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            try
            {
                ...
                Response.Redirect("add-kyc-docs.aspx?DocID=" + ID, false);
                HttpContext.Current.ApplicationInstance.CompleteRequest();
            }
            catch (Exception ex)
            {
                CE.StoreExceptionMessage("view-kyc-docs.aspx", "Repeater1_ItemCommand Edit", ex.Message, "Not Fixed");
            }
        }
        else if (e.CommandName == "Delete")
        {
            try
            {
                int ID = Convert.ToInt32(e.CommandArgument);
                int ret = 0;
                DataTable dt1 = KYC.ViewAllFlatCustomerKYCDocs("", "", "", Convert.ToString(ID), clientId);
                if (dt1 != null && dt1.Rows.Count > 0)
                {
                    string[] fileTypes = { "SaleDeedDraft", "SaleDeedFinal", "SaleAgreementDraft", "SaleAgreementFinal", "AllotmentLetter", "WelcomeLetter", "DemandLetter", "PaymentReceipt", "Bill", "PaymentSchedule", "EBCard", "HandingOverDocs", "CCUpdation", "NOCforHandover", "OtherDocuments", "BankDocuments" };
                    foreach (DataRow row in dt1.Rows)
                    {
                        foreach (string fileType in fileTypes)
                        {
                            if (!string.IsNullOrEmpty(Convert.ToString(row[fileType])))
                            {
                                string imagename = Convert.ToString(row[fileType]);
                                FileDelete(imagename, "KYCDocs");
                            }
                        }
                    }
                }
                ...
                ret = KYC.DeleteKYCByDocID(ID, Convert.ToString(clientId));
                if (ret == 1)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                      "Swal.fire({ " +
                       "  title: 'KYC docs details has been deleted as requested ', " +
                ...
                else
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert",
                      "Swal.fire({ " +
                      "  title: 'Your KYC docs details not deleted. please try again', " +
                      "  confirmButtonText: 'OK', " +
                       "  customClass: { " +
                        "    confirmButton: 'handle-btn-success' " +
                        "  } " +
            ...
            {
                CE.StoreExceptionMessage("view-kyc-docs.aspx", "Repeater1_ItemCommand Delete", ex.Message, "Not Fixed");
            }
        }
        else
        {
    ...
    public void FileDelete(string ImageName, string appkey)
    {
        ...
        if (System.IO.File.Exists(filePath))
        {
            ...
            {
                CE.StoreExceptionMessage("view-kyc-docs.aspx", "FileDelete", e.Message, "Not Fixed");
            }
        }
    }
    ...
    public void Bindblock()
    {
        ...
        {
            CE.StoreExceptionMessage("view-kyc-docs.aspx", "Bindblock", ex.Message, "Not Fixed");
        }
    }
    ...
    public void BindFlat()
    {
        ...
        {
            CE.StoreExceptionMessage("view-kyc-docs.aspx", "BindFlat", ex.Message, "Not Fixed");
        }
    }
