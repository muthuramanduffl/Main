...
public partial class adminkey2hcom_AddDemand2 : System.Web.UI.Page
{
    ...
    public void BindFltID(string ID)
    {
        ...
        try
        {
            if (Request.QueryString["Source"] != null && Request.QueryString["Source"] == "All")
            {
                dt = KC.ViewAllflatBookingCustomerDetails("", "", ID, "", "");
            }
            else
            {
                dt = Kdemand.ViewAllFlatDemandDetails("", "", "", ID);
            }
            if (dt != null && dt.Rows.Count > 0)
            {
                if (!string.IsNullOrEmpty(dt.Rows[0]["ProjectID"].ToString()) && dt.Rows[0]["ProjectID"] != null)
        ...
        {
            CE.StoreExceptionMessage("add-demand1.aspx", "Bind", ex.Message, "Not Fixed");
        }
    }
    ...
    public void FlatFunction()
    {
        ...
        {
            CE.StoreExceptionMessage("add-demand1.aspx", "FlatFunction", ex.Message, "Not Fixed");
        }
    }
    ...
    {
        Response.Redirect("add-payment-transaction1.aspx?FlatID=" + ddlFlatNumber.SelectedValue);
    }
